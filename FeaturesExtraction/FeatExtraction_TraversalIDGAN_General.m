function [T]= Looping_DVD_ADQ7_process_interface(folder_name, dim, manual_features, thresh_level)
...
for i=1:size(list,1)
   ...
   % Phase=imread('untitled.jpg')
    i
            ...
            cell_mask=im2bw(Phase,thresh_level);
            se = strel('disk',4,4);
...
%             
              ...
              if (length(str2mat(maskprops.Area)) == 1)
                   % se = strel('disk',8,8);
                    [feature, ALS, boundary, DMDFlucNorm ] = Process_FeatureExtraction_DMD_TraversalIDGAN_General(k, sysConfig, Intensity, cell_mask, scale );
              else
                    feature=zeros(1,manual_features)
              end
%                
             %j
            %[feature, ALS, boundary, DMDFlucNorm ] = Process_FeatureExtraction_DMD_TraversalIDGAN_General(k, sysConfig, Phase2d, cell_mask, scale );
...
% 
   %  end
    ...
    Label_mask = {'BulkA', 'BlukV', 'BulkC','BulkD', 'BulkE', 'BulkAR', 'BulkO'};%1-7
  ...
  %  Unit_bftextfib = {'um','', '', ''};
    ...
    Label_qp = {'GlobalInt1', 'GlobalInt2', 'GlobalInt3', 'GlobalInt4', 'GlobalInt5', 'GlobalInt6', 'GlobalCentDisp'};%49-55
    Unit_qp =  {'rad', 'rad^2', '', '', 'rad', 'rad','um'};
    Label_DMDVar={'LocalTextFit1','LocalTextFit2','LocalTextFit3','LocalTextFit4'};
    Unit_DMFit={'','','',''};
    Label_DC = {'LocalTextDC1', 'LocalTextDC2', 'LocalTextDC3', 'LocalTextDC4', 'LocalTextDC5', 'LocalTextDC6'};%56-61
    Unit_DC = {'rad', 'rad', '', '','um',''};
    Label_qptextc = {'LocalTextEnt1', 'LocalTextEnt2', 'LocalTextEnt3', 'LocalTextEnt4', 'LocalTextEnt5', 'LocalTextEnt6'};%62-67
    Unit_qptextc = {'','','','','',''};
    Label_qptextfib = {'LocalTextFib1', 'LocalTextFib2', 'LocalTextFib3', 'LocalTextFib4'};%68-71
    Unit_qptextfib = {'um','','',''};
    Label_qpDistri={'GlobalIntArr1', 'GlobalIntArr2', 'GlobalIntArr3', 'GlobalIntArr4', 'GlobalIntArr5'};%72-76
...
%     UnitList = [UnitList,Unit_qp, Unit_DM, Unit_DMDVar, Unit_DC, Unit_qptextc, Unit_qptextfib, Unit_qpDistri];
    ...
    %featureList = [featureList, Label_mask, Label_qp ,Label_DMDVar, Label_DC, Label_qptextc, Label_qptextfib, Label_qpDistri];
    %UnitList = [UnitList, Unit_qp, Unit_DM, Unit_DC, Unit_qptextc, Unit_qptextfib, Unit_qpDistri];
    
    featureList = [featureList, Label_mask, Label_qp , Label_DC, Label_qptextc, Label_qptextfib, Label_qpDistri];
    UnitList = [UnitList, Unit_qp, Unit_DM, Unit_DC, Unit_qptextc, Unit_qptextfib, Unit_qpDistri];
...
VarianceAllColNorm=normalize(VarianceAllCol)   
rowNorm=[];
...
rowScale=[];
for row=1:size(VarianceAllCol,1)
    rowScale=[rowScale; rescale(VarianceAllCol(row,:))];
end
%Add the desired normalized matrix for each set of traversals
%rowNorm or rowScale or VarianceAllColNorm or VarianceAllCol
T=array2table(VarianceAllCol);
T.Properties.VariableNames=featureList;
h=heatmap(rowScale,'Colormap', parula )
h.XData=featureList
ax=gca
ax.Colormap('jet')

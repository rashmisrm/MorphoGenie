...
from gan_training.utils import return_data
...
parser.add_argument('--dataset', default='QPI_Scaled/LC_Gray/', type=str, help='dataset name') #CellCycle/Vero/LC
parser.add_argument('--datatype', default='rgb', type=str, help='dataset name') # options are rgb, gray, cpa-which is overlay
...
train_dataset = get_dataset(
    ...
    data_dir=os.path.join(args.dset_dir, args.dataset),
...
if selectVAE=='dvae':
    ...
    fvae_ckpt_path = os.path.join('Z:/COVID-FTP/Rashmi/ID-GANCheckpoints/outputs', args.dvae_name,'chkpts', config['fvae']['chkptname'])
...
while not out:
    ...
    for x_real in data_loader:
        ...
        #x_real = x_real.to(dtype=torch.long, device=device)
        ...
        if it >= max_iter:
            ...
            break
        if it % 10 == 0:
            writer.add_scalar("GeneratorLoss", g_loss_last, it)
            writer.add_scalar("DiscriminatorLoss", d_loss_last, it)
